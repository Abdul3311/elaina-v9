...
const isOwner = [`${setting.ownerNumber}`,"62895333501279@s.whatsapp.net","6285156137901@s.whatsapp.net"].includes(sender) ? true : false
...
}, text: 'Nehh preview fotonya kalo males download' }, {quoted: quoted})
   } 
   }
}
...
conn.sendMessage(from, { caption: cap, image: { url: feta.data.qris }, buttons: buto, footer: '© created by ZansLord' })
}
console.log(feta)
}
break
...
if (!q) return reply('link githubnya mana?\n*Contoh:*\n#gitclone https://github.com/ZansLord/ElainaMultiDevice')
var linknya = q
...
.then(data => {
var opt = { packname: 'Elaina', author: 'By ZansLord' }
...
if (!isCmd) {
  ...
  if (m.messages[0x0].type == 'conversation' || m.messages[0x0].type == 'extendedTextMessage') {
    try {
      var chat_anonymous = m.messages[0x0].message.extendedTextMessage.text;
    } catch (_0x2d0d82) {
      var chat_anonymous = m.messages[0x0].message.conversation;
    }

    let text_nya_menfes = '*ANONYMOUS CHAT*\n💬 : ' + chat_anonymous;
    conn.sendMessage(cekPesan('teman', sender), {
